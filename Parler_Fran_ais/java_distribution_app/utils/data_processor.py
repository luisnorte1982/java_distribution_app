...
def raw_to_dataframes(raw: dict[str, list[dict]]) -> dict[str, pd.DataFrame]:
    ...
    # ── Reference tables ─────────────────────────────────────────────
    frames["items"] = pd.DataFrame(raw["items"])
    frames["vendors"] = pd.DataFrame(raw["vendors"])
    frames["dim_distribution"] = pd.DataFrame(raw["dim_distribution"])
    frames["customers_be"] = pd.DataFrame(raw["customers_be"])
    frames["customers_lu"] = pd.DataFrame(raw["customers_lu"])
