...
with st.sidebar:
    ...
    if "🔗" in data_mode:
        ...
        # Auto-load credentials from Streamlit secrets or environment variables
        _cid = BC_CLIENT_ID or ""
        try:
            import streamlit as _st
            _csecret = _st.secrets.get("BC_CLIENT_SECRET", "") or os.environ.get("BC_CLIENT_SECRET", "")
        except Exception:
            _csecret = os.environ.get("BC_CLIENT_SECRET", "")
        if _cid and _csecret:
            os.environ["BC_CLIENT_ID"] = _cid
            os.environ["BC_CLIENT_SECRET"] = _csecret
            st.success("✅ Credentials configurées automatiquement")
        else:
            st.warning("⚠️ Credentials BC manquantes dans les secrets Streamlit")
