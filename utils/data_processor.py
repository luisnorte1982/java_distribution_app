...
def raw_to_dataframes(raw: dict[str, list[dict]]) -> dict[str, pd.DataFrame]:
    ...
    # DEBUG - log actual columns from BC
    import streamlit as _st
    _st.write("🔍 Colonnes reçues de BC:", list(sales_docs.columns))
    # Try to find the document number column
    _no_col = None
    for _c in ["No", "No_", "Number", "Document_No", "Invoice_No"]:
        if _c in sales_docs.columns:
            _no_col = _c
            break
    if _no_col is None:
        _st.error(f"Colonne No introuvable. Colonnes disponibles: {list(sales_docs.columns)}")
        raise KeyError("No column not found")
    if _no_col != "No":
        sales_docs = sales_docs.rename(columns={_no_col: "No"})
...
def build_enriched_fact_table(frames: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    _st2.write("🔍 Colonnes items (10 premiers):", list(items.columns)[:10])
    # Find item number column
    _item_no = next((c for c in ["No", "No_", "Item_No", "Number", "Code"] if c in items.columns), None)
    if _item_no is None:
        _st2.error(f"Colonne No introuvable dans items. Colonnes: {list(items.columns)}")
        _item_no = items.columns[0]
    item_cols = [_item_no]
    for c in ["Description", "Marque_NSI", "Item_Category_Code", "Vendor_No", "Unit_Cost", "Unit_Price"]:
        if c in items.columns:
            item_cols.append(c)
    df = df.merge(
        items[item_cols].rename(columns={_item_no: "Item_No_item"}),
    ...
    _st3.write("🔍 Colonnes vendors (10 premiers):", list(vendors.columns)[:10])
    _vendor_no = next((c for c in ["No", "No_", "Vendor_No", "Number", "Code"] if c in vendors.columns), None)
    if _vendor_no is None:
        _vendor_no = vendors.columns[0]
    vendor_cols = [_vendor_no]
    for c in ["Name", "Location_Code", "Country_Region_Code"]:
        if c in vendors.columns:
            vendor_cols.append(c)
    df = df.merge(
        vendors[vendor_cols].rename(columns={_vendor_no: "Vendor_No_v", "Name": "Vendor_Name", "Location_Code": "Vendor_Location"}),
